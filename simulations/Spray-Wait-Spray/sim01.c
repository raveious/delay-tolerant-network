/*
 * sim01.c
 * Delayed Utility - Spray, Wait, Spray
 * 
 */
...
#define UTILTY_T 40 // Utility Threshold
#define TIMECONVERT 1.5 // Time convertion factor
...
/*
 * Arguments: "Range", "Number of run", "Number of Copies", "Number of Nodes", "Grid Width", "Number of Copy stacks"
 *
 *
 */
...
int main(int argc, char *argv[]) {				
	...
	simSize = atoi( argv[5] );	
	divider = atoi( argv[6] );
	//printf("We have %d copies and a divider of %d\n", srcCopies, divider);
	...
	while( run < totalTrials ){
		...
		for( k=0; k<numNodes; k++ ){
			...
			motes[k].msgCopies = 0;		
			motes[k].utilityThreshold = UTILTY_T;
			for( w=0; w<numNodes+1; w++ ){
				motes[k].nodesSeen[w] = 0;
			...
			for( k=0; k<numNodes; k++ ){
				...
				if( motes[k].srcFlag != -1 ){
					...
					if( motes[k].inMsg != 0 ){
						...
						if( motes[k].status == 1 ){
							motes[k].nodesSeen[ motes[k].inMsg->currSrcID ] = SEEN_TIME_MAX;
						...
						if( motes[k].helloTimer == 0 ){
							...
							if( motes[k].status == 1 ){
								...
								motes[k].helloMsg.currDestID = -1;
								if( motes[k].msgCopies > 1 ){
									motes[k].helloMsg.numCopies = motes[k].msgCopies / divider;
								}
								else{
									motes[k].helloMsg.numCopies = 0;
								}
					...
					if( motes[k].inMsg != 0 ){
						...
						if( (motes[k].status == 1) ){
							...
							/* If message is a relay message from either source or relay */
								...
								if( motes[k].status == 0 ){
									...
									if( motes[k].nodesSeen[ motes[k].inMsg->finalDestID ] < UTILTY_T ){
										motes[k].status = 2;
									...
									if( motes[k].inMsg->srcID == motes[k].inMsg->currSrcID ){
										...
										if( (motes[k].inMsg->numCopies > 0) ){
											...
											if( motes[k].status == 2 ){
												//printf("****TEST DONE****Relay heard from Source, Status 2\n");
												motes[k].helloMsg.numCopies = motes[k].inMsg->numCopies;
											}
											else{
												//printf("****TEST DONE****elay heard from Source, Status 3\n");
												motes[k].helloMsg.numCopies = motes[k].inMsg->numCopies / 2;
											}
											motes[k].msgCopies = motes[k].inMsg->numCopies;
										...
										if( motes[k].status == 2 ){
											motes[k].helloMsg.numCopies = motes[k].inMsg->numCopies;		
											//printf("****TEST DONE****Relay heard from relay, Status 2\n");
										}
										else{
											//printf("****TEST DONE****Relay heard from relay, Status 3\n");
											motes[k].helloMsg.numCopies = motes[k].inMsg->numCopies / 2;
										}
										motes[k].msgCopies = motes[k].inMsg->numCopies;
								...
								/* I don't have current relay message, add new message to deliver */
									...
									/* Message recieved was from another relay node, grab message if possible */
							...
							/* Got ACK from destination node, got back to saying I'm Here Hello */
								...
								if( motes[k].inMsg->currSrcID == motes[k].helloMsg.finalDestID ){
									...
									if( motes[k].msgCopies > 0 ){
										if( (motes[k].status == 2) || (motes[k].msgCopies == 1) ){
											motes[k].msgCopies = 0;
											motes[k].helloMsg.numCopies = 0;
											//printf("****TEST DONE****Relay heard ACK, no more copies\n");
										}
										else{
											
											motes[k].helloMsg.numCopies = motes[k].inMsg->numCopies / 2;
											//printf("****TEST DONE****Relay heard ACK, 1/2 copies\n");
										}									
						...
						if( motes[k].status == 1 ){
							...
							if( (motes[k].helloTimer == 0) && (bufferPtr == -1) ){
								...
								if( motes[k].msgCopies != 0 ){
									motes[k].helloMsg.numCopies = srcCopies / divider;
								}
								else{
									motes[k].helloMsg.numCopies = 0;
								}
