...
#define SPRAY_DELAY 8
#define TIMECONVERT 1.5
...
struct node {
	...
	struct message delivery; 
	struct message msgBuff[SIZE_MAX]; /* Buffer of outgoing messages */
	
};

void resetMsg( struct message *msg, int k ){
	msg->msgID = 0;
	msg->msgType = 0;
	msg->srcID = -1;
	msg->finalDestID = -1;
...
int main(int argc, char *argv[]) {				
	...
	if( f != NULL ){
		...
		while( run < totalTrials ){
			...
			for( k=0; k<numNodes; k++ ){
				...
				resetMsg( &motes[k].helloMsg, k );			
				resetMsg( &motes[k].delivery, k );						
			...
			motes[srcID].msgCopies = srcCopies;
				...
				for( k=0; k<numNodes; k++ ){
					...
					if( motes[k].srcFlag != -1 ){
						...
						if( startTime == currTime ){
							motes[k].status = 1;
							motes[k].delivery.msgID = 1;
							motes[k].delivery.msgType = 1;
							motes[k].delivery.srcID = k;
							motes[k].delivery.finalDestID = motes[k].dstID;
							motes[k].delivery.currSrcID = k;
							motes[k].delivery.currDestID = -1;								
							motes[k].delivery.numHops = 0;														
						}			
						/* Recieved Message */	
						if( motes[k].inMsg != 0 ){							
							bufferPtr = -1;
							...
							if( motes[k].status == 1 ){
								...
								if( motes[k].inMsg->msgType == 0 ){
									...
									if( motes[k].msgCopies > 0 ){
										...
										motes[k].msgBuff[ bufferPtr ].srcID = motes[k].delivery.srcID;
										motes[k].msgBuff[ bufferPtr ].finalDestID = motes[k].delivery.finalDestID;
										...
										motes[k].msgBuff[ bufferPtr ].numCopies = motes[k].msgCopies;										
									}
									else{								
										if( motes[k].inMsg->currSrcID == motes[k].delivery.finalDestID ){
											...
											motes[k].msgBuff[ bufferPtr ].currDestID = motes[k].inMsg->currSrcID;
											motes[k].msgBuff[ bufferPtr ].numHops = 1;
											motes[k].msgBuff[ bufferPtr ].numCopies = -1;
											motes[k].buffExpire[ bufferPtr ] = ACK_WAIT;											
										}
									}	
								}
						...
						if( motes[k].inMsg != 0 ){
							...
							if( (motes[k].status == 0) && (motes[k].inMsg->finalDestID == k) 
								...
								if( motes[k].inMsg->currSrcID == motes[k].inMsg->srcID ){
									printf("*****Dst got message from Source \n");
									testTimeDataSrcToDst[run] = currTime;									
								}	
								else{
									printf("*****DST got message from Relay Node: \n%d", motes[k].inMsg->currSrcID );
									testTimeDataSrcToDst[run] = currTime;							
								}							
								avgNumHopsSrctoDst[run] = motes[k].inMsg->numHops + 1;								
								done = 1;
							...
							if( (motes[k].status == 1) ){
								...
								if( motes[k].inMsg->msgType == 0 ){
									...
									if( motes[k].delivery.finalDestID == motes[k].inMsg->currSrcID  ){
										motes[k].msgBuff[ bufferPtr ].msgID = 1;
										motes[k].msgBuff[ bufferPtr ].msgType = 1;
										motes[k].msgBuff[ bufferPtr ].srcID = motes[k].delivery.srcID;
										motes[k].msgBuff[ bufferPtr ].finalDestID = motes[k].delivery.finalDestID;
										motes[k].msgBuff[ bufferPtr ].currSrcID = k;
										motes[k].msgBuff[ bufferPtr ].currDestID = motes[k].delivery.finalDestID;
										motes[k].msgBuff[ bufferPtr ].numHops = motes[k].delivery.numHops + 1;
										motes[k].buffExpire[ bufferPtr ] = ACK_WAIT;
									}
								}
								/* If message is a relay message from either source or relay */
									...
									if( motes[k].inMsg->srcID == motes[k].inMsg->currSrcID ){
										...
										if( motes[k].inMsg->numCopies > 0 ){
											motes[k].delivery.msgID = motes[k].inMsg->msgID;
											motes[k].delivery.msgType = 1;
											motes[k].delivery.srcID = motes[k].inMsg->srcID;
											motes[k].delivery.finalDestID = motes[k].inMsg->finalDestID;
											motes[k].delivery.currSrcID = k;
											motes[k].delivery.currDestID = motes[k].inMsg->finalDestID;
											motes[k].delivery.numCopies = -1;
											motes[k].delivery.numHops = motes[k].inMsg->numHops + 1;;		
			...
			//fprintf(f, "Final Results:\nResults Src to Dst Time,%d,\n", testTimeDataSrcToDst[0]);				
			run++;		
			sleep(1);
		}
	...
	finalAvg = 0;
	for( i=0; i<run; i++){
		finalAvg = finalAvg + trafficAvg[i];
	}	
	finalAvg = finalAvg / run;
	fprintf(f, "Avg Number of traffic from Src to Dst,%d\n", finalAvg);
